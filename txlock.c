...
// private unprofiled lock methods for
// internal use in condition variables
static int _tas_lock(tas_lock_t *l) {
    if (tatas(&l->val, 1)) {
        int s = spin_begin();
        do {
            s = spin_wait(s);
...
static int _tas_unlock(tas_lock_t *l) {
    __sync_lock_release(&l->val);
    return 0;
}
...
int tc_signal(txcond_t* cond_var){
  ...
  if(i%10==0){
    if(cv->head == NULL){ _tas_unlock(&cv->lk); return 0;}
    node = cv->head;
    ...
    if(cv->tail == NULL){ _tas_unlock(&cv->lk); return 0;}
    node = cv->tail;
    cv->tail = node->prev;
